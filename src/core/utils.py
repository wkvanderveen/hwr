...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.3,
    iou_thresh=0.5):
    """
...
def py_nms(boxes, scores, max_boxes=50, iou_thresh=0.5):
    ...
    Arguments: boxes => shape of [-1, 4], the value of '-1' means that
                        dont know the exact number of boxes
               scores => shape of [-1,]
               max_boxes => representing the maximum of boxes to be
               selected by non_max_suppression
               iou_thresh => representing iou_threshold for deciding to
               keep boxes
    """
...
def cpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.3,
            iou_thresh=0.5):
    """
    Arguments:
...
def draw_boxes(image, boxes, scores, labels, classes, detection_size,
    ...
    for i, box in enumerate(boxes):
        for io, other in enumerate(picked_boxes):
            if box is other:
                continue
            if min_dist > abs((box[0]+box[2])/2 - (other[0]+other[2])/2):
                break
        else:
    ...
    for i in range(len(boxes)):  # for each bounding box, do:
        ...
        bbox_text = f"{label} {score:.2f}"
        text_size = draw.textsize(bbox_text, font)
        # convert_to_original_size
        # detection_size, original_size = \
        #     np.array(detection_size), np.array(image.size)
...
def freeze_graph(sess, output_file, output_node_names):
    ...
    print(f"=> {len(output_graph_def.node)} ops written to {output_file}.")
        ...
        if 'Conv' in var1.name.split('/')[-2]:
            ...
            if 'BatchNorm' in var2.name.split('/')[-2]:
                ...
                assign_ops.append(tf.assign(
                    bias, bias_weights, validate_shape=True))
...
def evaluate(y_pred, y_true, iou_thresh=0.5, score_thresh=0.3):
    ...
    for i in range(num_images):
        ...
        if len(pred_labels_list) != 0:
            for cls, count in Counter(pred_labels_list).items():
                pred_labels_dict[cls] += count
        else:
            continue
        ...
        for k in range(len(pred_labels_list)):
            ...
            if (iou[m] >= iou_thresh
                    and pred_labels_list[k] == true_labels_list[m]
                    and m not in detected):
